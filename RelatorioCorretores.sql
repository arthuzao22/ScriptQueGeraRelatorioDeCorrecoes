...
/*VALIDAÇÃO*/
/*VERIFICA SE HÁ ALGUM CPF DUPLICADO, CASO TIVER, O SELECT ABAIXO IRÁ MOSTRAR*/
SELECT NU_CPF, COUNT(*) as contagem
FROM #RESULTADO_FINAL_CORRETORES
GROUP BY NU_CPF
HAVING COUNT(*) > 1;

/*REMOVE TODOS CPF DUPLICADOS*/
WITH CTE AS (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY NU_CPF ORDER BY NU_CPF) AS rn
    FROM #RESULTADO_FINAL_CORRETORES
)
